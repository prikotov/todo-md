...
final class Board
{
    ...
    private static function renderTaskSkeleton(string $id, string $title, string $type, array $opts, string $status): string
    {
        ...
        $lines[] = '';
        $lines[] = '### История (User Story)';
        $lines[] = '> (заполнить)';
        $lines[] = '';
        $lines[] = '### Цель по SMART (Goal)';
        $lines[] = '- (заполнить)';
        $lines[] = '';
        ...
        $lines[] = '```';
        $lines[] = '';
        $lines[] = '## 7. Риски и зависимости (Risks and Dependencies)';
        $lines[] = '- (заполнить)';
        $lines[] = '';
        $lines[] = '## 8. Источники (Sources)';
        $lines[] = '';
        $lines[] = '## 9. Комментарии (Comments)';
        $lines[] = '';
    ...
    private static function renderEpicSkeleton(string $id, string $title, array $opts): string
    {
        ...
        $lines[] = '';
        $lines[] = '## 3. Требования, MoSCoW (Requirements)';
        $lines[] = '### 🔴 Блокирующие требования (Must Have)';
        $lines[] = '- [ ] (заполнить)';
        $lines[] = '### ⚫ Won\'t Have (Не в этот раз)';
        $lines[] = '- (заполнить)';
        $lines[] = '';
        $lines[] = '## 4. Техническое решение (Solution Design)';
        $lines[] = '';
        $lines[] = '## 5. План реализации (Implementation Plan)';
        $lines[] = '';
        $lines[] = '## 6. Критерии приёмки эпика (Definition of Done)';
        $lines[] = '- [ ] (заполнить)';
        $lines[] = '';
        $lines[] = '## История изменений (Change History)';
        $lines[] = '| Дата | Автор (роль) | Изменение |';
