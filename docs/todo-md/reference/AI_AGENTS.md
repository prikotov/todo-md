...
Валидатор `todo-md validate` проверяет, что агент в полях `author`/`assignee` входит в этот список. Проект расширяет список собственными агентами и ролями через конфиг `.todo-md.php` — см. [CONFIG.md](./CONFIG.md).
