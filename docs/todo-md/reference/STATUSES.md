...
- **done**: Работа завершена исполнителем и представлена к приёмке. Момент перехода в `done` определяется Git-процессом проекта.
...
Диаграмма отражает порядок статусов, а не момент перехода относительно merge: `done` — работа завершена исполнителем и представлена к приёмке; момент перехода в `done` определяется Git-процессом проекта.
...
5.  **Завершение:** Статус меняется на `done`, файл перемещается в `todo/done/`. `done` — работа завершена исполнителем и представлена к приёмке; момент перехода определяется Git-процессом проекта (например, в процессах с апрувом PR — до запроса апрува). Возврат из `done` допустим (например, в `review` при замечаниях ревью) — CLI его не запрещает.
