...
- **done**: Работа завершена исполнителем и представлена к приёмке (acceptance). Момент перехода в `done` определяется Git-процессом проекта.
...
Диаграмма отражает порядок статусов, а не момент перехода относительно merge: `done` — см. определение выше.
...
5.  **Завершение:** Статус меняется на `done`, файл перемещается в `todo/done/`. Момент перехода — см. определение выше (например, в процессах с апрувом PR — до запроса апрува). Возврат из `done` допустим (например, в `review` при замечаниях ревью) — CLI его не запрещает.
