...
Самодостатчный HTML-дашборд из `.todo.md` задач — канбан-доска, графики и диаграмма Ганта в одном статичном файле (Chart.js подгружается с CDN). Без сервера: открывается по `file://` и обновляется перегенерацией.
